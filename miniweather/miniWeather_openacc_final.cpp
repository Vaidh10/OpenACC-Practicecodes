...
int main(int argc, char **argv)
{
  ...
  nvtxRangePushA("init");
  init();
  #pragma acc data copyin(state_tmp[(nz + 2 * hs) * (nx + 2 * hs) * NUM_VARS], hy_dens_cell[nz + 2 * hs], \
    hy_dens_theta_cell[nz + 2 * hs], hy_dens_int[nz + 1], hy_dens_theta_int[nz + 1], hy_pressure_int[nz + 1]) \
        ...
        copy(state [0:(nz + 2 * hs) * (nx + 2 * hs) * NUM_VARS])
  {
      ...
      printf("\n Complete. Execution time is = %lf seconds\n",  exe_time);
  }
  nvtxRangePop();
  finalize();
  nvtxRangePop();
...
void semi_discrete_step(double *state_init, double *state_forcing, double *state_out, double dt, int dir, double *flux, double *tend)
{
 ...
 #pragma acc parallel loop gang vector collapse(3) default(present)
...
void compute_tendencies_x(double *state, double *flux, double *tend)
{
  ...
  #pragma acc parallel loop gang vector collapse(2) private(stencil,d3_vals,vals) default(present)
  for (k = 0; k < nz; k++)
  {
    for (i = 0; i < nx + 1; i++)
    {
...
void set_halo_values_x(double *state)
{
  ...
  double z;
  #pragma acc parallel loop gang vector collapse(2) default(present)
  for (ll = 0; ll < NUM_VARS; ll++)
  {
    for (k = 0; k < nz; k++)
    {
